...
if __name__ == "__main__":
    # Build the index based on offenders.csv using the 2nd column as the result (the "document")
    # and all other columns as data. All values in the csv will be passed to the filter function
    # to get them ready for entry in the index.
    index = ii.iindex_build_csv("sample.csv", document_column=2, filt=filter_non_alphanumeric)

    # REPL!
    while True:
        # If you try to exit with ctrl-c or ctrl-d, exit gracefully w/o traceback
        try:
            search = input("Enter search terms for sample.csv\n> ")
